...
export default function SalesPage() {
  const { sales, deleteSale, financials } = useCRMData();
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in pb-12 max-w-4xl mx-auto">
      ...
      <div className="p-5 sm:p-6 rounded-2xl glass-panel border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Manual sales tracking with product costs, Meta ads, and partner attribution.
          </p>
        </div>

        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-3 rounded-xl text-xs font-bold shadow-lg shadow-emerald-600/30 active:scale-95 transition-all cursor-pointer"
        >
      ...
      <div className="space-y-2.5">
        <div className="flex items-center gap-2">
          {(['All', 'Adem', 'Abdou'] as const).map((tab) => (
            ...
            >
              {tab === 'All' ? `All Sales (${sales.length})` : `${tab} (${sales.filter((s) => s.soldBy === tab).length})`}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-3">
        {filteredSales.length === 0 ? (
          <div className="p-8 text-center glass-panel rounded-2xl border border-white/10 text-xs text-slate-400 space-y-3">
            <p>No sales found.</p>
            <button
              onClick={() => setIsModalOpen(true)}
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-slate-950/80 p-2.5 rounded-xl border border-white/5">
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase">Product Purchase Cost</span>
                    <span className="font-semibold text-slate-200">
                      ${sale.productCostUsd} <span className="text-[10px] text-slate-400">({productCostDzd} DA)</span>
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase">Meta Ad Spend Cost</span>
                  ...
                  <div className="flex items-center gap-2">
                    {sale.customerPhone && (
                      <a
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-600/20 text-emerald-400 text-xs font-bold border border-emerald-500/30 hover:bg-emerald-600/30 active:scale-95"
                      >
      ...
      <FastSaleModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  );
}
